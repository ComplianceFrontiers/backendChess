...
from app.database import form_Bear_Middletown_Chess_Tournament
...
form_Bear_Middletown_Chess_Tournament_bp = Blueprint('form_Bear_Middletown_Chess_Tournament', __name__)
...
def generate_unique_profile_id():
    while True:
        ...
        if form_Bear_Middletown_Chess_Tournament.count_documents({"profile_id": profile_id}) == 0:
            return profile_id

@form_Bear_Middletown_Chess_Tournament_bp.route('/form_Bear_Middletown_Chess_Tournament_bp_submit', methods=['POST'])
def form_Bear_Middletown_Chess_Tournament_bp_submit():
    try:
        ...
        # Insert into MongoDB
        form_Bear_Middletown_Chess_Tournament.insert_one(form_data)
...
@form_Bear_Middletown_Chess_Tournament_bp.route('/form_Bear_Middletown_Chess_Tournament_bp_delete_records_by_profile_ids', methods=['DELETE'])
def form_Bear_Middletown_Chess_Tournament_bp_delete_records_by_profile_ids():
    try:
        ...
        for profile_id in profile_ids:
            ...
            result = form_Bear_Middletown_Chess_Tournament.delete_one({"profile_id": str(profile_id)})
...
@form_Bear_Middletown_Chess_Tournament_bp.route('/get_forms_form_Bear_Middletown_Chess_Tournament', methods=['GET'])
def get_forms_form_Bear_Middletown_Chess_Tournament():
    try:
        # Fetch all documents from the collection in descending order
        records = list(form_Bear_Middletown_Chess_Tournament.find({}, {'_id': 0}).sort([('_id', -1)]))  # Sort by '_id' in descending order
...
@form_Bear_Middletown_Chess_Tournament_bp.route('/form_Bear_Middletown_Chess_Tournament_by_profile_id', methods=['GET'])
def get_form_Bear_Middletown_Chess_Tournament_by_profile_id():
    try:
        ...
        record = form_Bear_Middletown_Chess_Tournament.find_one({"profile_id": str(profile_id)}, {'_id': 0})  # Exclude MongoDB's default '_id' field
